...
export default function SelectPlan(props: Props) {
    ...
    return (
        <div aria-disabled className={styles}>
            {props.recommended && <div className={"bg-success text-xs w-28 text-center rounded-sm absolute top-0 left-8"}>
                {props.offerString}
            </div>}
            {props.expired && <div className={"bg-danger2 text-xs w-28 text-center rounded-sm absolute top-0 left-8"}>
                {props.offerString}
            </div>}
